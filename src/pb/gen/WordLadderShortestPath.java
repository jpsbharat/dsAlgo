...
public class WordLadderShortestPath {
    ...
    public static void main(String[] args) {
        ...
        String endWord = "DOG";
        Ladder result = getShortestTransformationIterative(startWord, endWord,
                dictionary);
        // Ladder result = getShortestTransformationRecursive(startWord,
        // endWord, dictionary);
        if (result != null) {
            System.out.println("Length is " + result.getLength()
                    + " and path is :" + result.getPath());
        } else {
            System.out.println("No Path Found");
        }
    }

    public static Ladder getShortestTransformationIterative(String startWord,
            ...
            while (!queue.isEmpty() && !queue.peek().equals(endWord)) {
                ...
                while (i.hasNext()) {
                    ...
                    if (differByOne(string, ladder.getLastWord())) {
                        ...
                        // later processing.
                        queue.add(new Ladder(list, ladder.getLength() + 1,
                                string));
    ...
    public static Ladder getShortestTransformationRecursive(String startWord,
                                                            String endWord, Set<String> dictionary) {
        ...
        List<String> path = new LinkedList<String>();
        path.add(startWord);
        recursiveHelperShortest(startWord, endWord, dictionary, new Ladder(
                path, 1, startWord), allPath, shortestPath);

        return shortestPath;
    }

    private static void recursiveHelperShortest(String startWord,
                                                String endWord, Set<String> dictionary, Ladder ladder,
        ...
        if (ladder.getLastWord().equals(endWord)) {
            ...
            if (shortestPath.getPath() == null
                    || shortestPath.getPath().size() > ladder.getPath().size()) {
        ...
        while (i.hasNext()) {
            ...
            if (differByOne(string, ladder.getLastWord())
                    && !ladder.getPath().contains(string)) {
